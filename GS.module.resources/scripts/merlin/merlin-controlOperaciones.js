$(document).ready(function () {
	...
	if(parseInt($(window).innerWidth())<990)
		Showfilter();
	setTimeout(ListarOperaciones, 1000);
...
function ListarDocumentos() {
    $("#filtroDocumentos").kendoDropDownList({
        filter: "contains",
        dataTextField: "v02",
        dataValueField: "v01",
        dataSource: {
            ...
            transport: {
                read: function (options) {
                    $.ajax({
                        type: "POST",
                        url: wsnode + "wsMaster.svc/ListarDocumentos",
...
function Showfilter()
{
    if ($('#filterValue').val() == '1')
        $('#filterValue').val('0');
    else
        $('#filterValue').val('1');

    if ($('#filterValue').val() == '0')
        $('#filterRow').hide();
    else
        $('#filterRow').show();           
...
function ListarOperaciones() {
    ...
    "documento": $("#filtroDocumentos").val(), 
	"orden": $("#filtroOp").val(), 
    ...
    $("#gridpadre").kendoGrid({
        ...
        transport: {
            read: function (options) {
                $.ajax({
                    type: "POST",
                    url: wsnode + "wsMaster.svc/ListarOperacionesTesoreriaALL",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    data: params,
                    async: false,
        ...
        rowTemplate:
            ...
            "<div class='col-xs-12 col-sm-6 col-md-4 pad-none' style='max-width: 286px;'>" +
            "<div class=\"home-buttom\" style='#: v15 #' onclick=\"AprobarOperacionId('#: v11 #','#: v12 #','#: v16 #');return false;\">" +
...
function AprobarOperacionId(Op, Db, documento)
{
