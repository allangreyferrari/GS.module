...
using GS.module.entities.Internal;
using Microsoft.Practices.EnterpriseLibrary.Data.Sql;

namespace GS.module.data.access.Configuration
{
    public class dao
    {
        public IDataReader GetCollectionIReader(string connectionStr, string method, object[] input, int timeout = 0)
        {
            Database db;
            IDataReader dr;
            try
            {
                ...
                using (DbConnection connection = db.CreateConnection())
                {
                    ...
                    DbCommand dbCommand;
                    if (input.Count() == 0)
                        dbCommand = db.GetStoredProcCommand(method);
                    else
                        dbCommand = db.GetStoredProcCommand(method, input.ToArray());
                    if (timeout != 0)
        ...
        public IDataReader GetCollectionIReader(Query query) 
        {
            ...
            try
            {
                db = new SqlDatabase(query.Connection);
                using (DbConnection connection = db.CreateConnection())
                {
                    connection.Open();
                    DbCommand dbCommand;
        ...
        public System.Text.StringBuilder GetCollectionJSON(Query query) 
        {
            ...
            Database db = DatabaseFactory.CreateDatabase(query.Connection);
            using (DbConnection connection = db.CreateConnection())
            {
                connection.Open();
        ...
        {
            Database db = new SqlDatabase(query.Connection);
            int result;
            using (DbConnection connection = db.CreateConnection())
            {
                connection.Open();
